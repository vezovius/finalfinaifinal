...
async def gender_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [[KeyboardButton("Yavru")], [KeyboardButton("Genç")], [KeyboardButton("Yetişkin")], [KeyboardButton("Yaşlı")]]
    reply_markup = ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True)
    await update.message.reply_text("Hayvanın yaş grubunu seçiniz:", reply_markup=reply_markup)
    return AGE
