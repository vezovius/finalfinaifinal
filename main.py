...
@bot.message_handler(
    func=lambda message: message.text in ["Köpek", "Kedi", "Kuş"])
...
@bot.message_handler(
    func=lambda message: message.text in ["Evet, kısır", "Hayır, değil"])
...
@bot.message_handler(
    func=lambda message: "şikayet" not in user_data.get(message.chat.id, {}))
...
@bot.message_handler(
    func=lambda message: "ilaçlar" not in user_data.get(message.chat.id, {}))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    keyboard = [[KeyboardButton("🐶 Köpek")], [KeyboardButton("🐱 Kedi")],
                [KeyboardButton("🐦 Kuş")]]
    reply_markup = ReplyKeyboardMarkup(keyboard, one_time_keyboard=True)
    await update.message.reply_text(
        "Veteriner botuna hoş geldiniz! Lütfen hayvan türünü seçiniz:",
        reply_markup=reply_markup)
    return TYPE


async def type_handler(update: Update,
                       context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data["type"] = update.message.text
    keyboard = [[KeyboardButton("Erkek")], [KeyboardButton("Dişi")]]
    reply_markup = ReplyKeyboardMarkup(keyboard, one_time_keyboard=True)
    await update.message.reply_text("Hayvanın cinsiyetini seçiniz:",
                                    reply_markup=reply_markup)
    return GENDER


async def gender_handler(update: Update,
                         context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def age_handler(update: Update,
                      context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data["age"] = update.message.text
    keyboard = [[KeyboardButton("Evet")], [KeyboardButton("Hayır")]]
    reply_markup = ReplyKeyboardMarkup(keyboard, one_time_keyboard=True)
    await update.message.reply_text("Hayvan kısırlaştırılmış mı?",
                                    reply_markup=reply_markup)
    return NEUTERED


async def neutered_handler(update: Update,
                           context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def complaint_handler(update: Update,
                            context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def medication_handler(update: Update,
                             context: ContextTypes.DEFAULT_TYPE) -> int:
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            TYPE:
            [MessageHandler(filters.TEXT & ~filters.COMMAND, type_handler)],
            GENDER:
